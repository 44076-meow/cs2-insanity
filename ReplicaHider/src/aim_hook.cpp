...
// Sigscan pattern — same as gamedata/replica.json:CCSBot_UpdateLookAngles.
...
extern "C" void aim_hook_handler(void* ccsbot) {
    ...
    void* pawn = nullptr;
    memcpy(&pawn, base + ReplicaHider::CCSBOT_PLAYER_PTR_OFFSET, sizeof(pawn));
    ...
    // advantage helped).
    float btPitch = *reinterpret_cast<float*>(base + ReplicaHider::CCSBOT_LOOK_PITCH_OFFSET);
    float btYaw   = *reinterpret_cast<float*>(base + ReplicaHider::CCSBOT_LOOK_YAW_OFFSET);
    ...
    *reinterpret_cast<float*>(base + ReplicaHider::CCSBOT_LOOK_PITCH_OFFSET) = pitch;
    *reinterpret_cast<float*>(base + ReplicaHider::CCSBOT_LOOK_YAW_OFFSET)   = yaw;
    ...
    if (pawn) {
        ...
        *reinterpret_cast<float*>(p + ReplicaHider::PAWN_EYE_ANGLES_OFFSET + 0) = pitch;
        *reinterpret_cast<float*>(p + ReplicaHider::PAWN_EYE_ANGLES_OFFSET + 4) = yaw;
