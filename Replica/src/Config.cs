...
public sealed class Config
{
    ...
    private static readonly string DefaultRealSteamIds =
        Path.Combine(Server.GameDirectory, "csgo", "addons", "counterstrikesharp",
            "configs", "plugins", "Replica", "real_steamids.txt");
    ...
    public string SteamIdMode      => Get("replica_steamid_mode", "synthetic");
    public string RealSteamIdsFile => Get("replica_real_steamids_file", DefaultRealSteamIds);
    ...
    public string TelemetryPath    => Resolve(Get("replica_telemetry_path",
        Path.Combine(LogsRoot, "replica", "{date}_{session}.jsonl")));
