...
public sealed class ReplicaPaintsPlugin : BasePlugin
{
    public override string ModuleName    => "ReplicaPaints";
    ...
    public override void Load(bool hotReload)
    {
        ...
        RegisterListener<Listeners.OnClientDisconnect>(slot => {
            ...
            // disconnect on mapchange / Replica kick, and we don't want
            // ropz's 1337 frags to vanish if the process dies right after.
            try { _players?.Save(); } catch { }
            try { _bots?.Save();    } catch { }
        });

        AddCommand("css_replica_paints_reload", "ReplicaPaints: hot-reload settings + catalogs + players", OnReloadCommand);
    ...
    /// admin issues <c>css_replica_paints_reload</c> from the server
    ...
    ///   - Managed Replica bots → BotsStore keyed by persona name. Bots
