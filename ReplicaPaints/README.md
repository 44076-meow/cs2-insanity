...
Third plugin in the [`replica`](../) stack. Slaps custom paintkits,
...
- **Replica-managed bots** — every loadout axis is derived
...
top-level `replica` README for the full warning.

## How it talks to Replica

`ReplicaPaints` is a **read-only** consumer of the shared mmap pool
at `/tmp/replica_fake_slots.bin`. It reads two things from the
pool:

- `managed[120]` — distinguishes Replica-managed bots from `bot_add`
...
- The 32-byte UTF-8 persona name slot for each managed bot (offset
  ...
  synchronously by Replica before the slot is marked managed, so by
...
No writes — the pool is owned by Replica, and we never touch any byte
...
| Command                      | Purpose                                                                                   |
| ---------------------------- | ----------------------------------------------------------------------------------------- |
| `css_replica_paints_reload` | Hot-reload `settings.json`, all catalogs, `players.json`, `bots.json`. No restart needed. |

Server console can always call it; in-game callers need the admin
flag. An `ReplicaPaints` reload is roster-neutral — Paints doesn't
own bot slots, so this command never produces ghost-slot
accumulation. (Earlier docs warned about ghost slots from
`css_plugins reload Replica`; that was fixed in
`fd9a478fc0e4c4a5ece1573380eb292b1e956995` by threading `hotReload`
through `FakeClientManager.OnUnload`.)

## Configuration files

All under `csgo/addons/counterstrikesharp/configs/plugins/ReplicaPaints/`.
...
ships in `gamedata/ReplicaPaints.json`. CSSharp resolves it lazily
...
Output goes to `bin/Release/net8.0/ReplicaPaints.dll` plus the
`wwwroot/` tree copied alongside it. The plugin is deployed to
`csgo/addons/counterstrikesharp/plugins/ReplicaPaints/` with
`gamedata/ReplicaPaints.json` inside it. Catalogs and the
`settings.json` template land in
`csgo/addons/counterstrikesharp/configs/plugins/ReplicaPaints/`.
...
- **Hot-reload ghost-slot accumulation** — fixed in
  ...
  Previously, `css_plugins reload Replica` (or a full
  ...
  the `Despawn` loop. Reloading **only** `ReplicaPaints` was
  never affected — Paints does not own bot slots — so the
  previously-documented `css_plugins reload ReplicaPaints`
...
- **Bot-psychology-driven skin selection.** Read `BotProfile.PsychologyType`
  from Replica (would be a new `psy_type[120]` field in the pool mmap
  — layout v8 bump on Replica's side) and use it as an extra seed in
